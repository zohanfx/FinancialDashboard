...
def plot_data(data, indicators, sync_axis=None):
    ...
    for indicator in indicators:
        if indicator == "30 Day SMA":
            ...
            p.legend_label.location = "top_left"
            p.legend_label.click_policy = "hide"
...
date_picker_from = DatePicker(title="Start Date", value="2020-01-01", min_date="2020-01-01",
                              max_date=dt.datetime.now().strftime("%Y-%m-%d"))

date_picker_to = DatePicker(title="End Date", value="2020-02-01", min_date="2020-01-01",
                              max_date=dt.datetime.now().strftime("%Y-%m-%d"))
